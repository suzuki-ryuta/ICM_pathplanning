...
std::vector<int> RRTTree::get_neighbors(int index, double radius)
{
	...
	Node center = graph[index].node;
	for(int i = 0; i < (int)graph.size(); ++i){
		if(i == index) continue;
		if(graph[i].is_valid && graph[i].distance(center) <= radius){
